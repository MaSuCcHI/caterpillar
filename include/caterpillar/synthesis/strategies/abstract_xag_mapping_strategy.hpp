...
using abs_steps_xag_t =
    std::vector<std::pair<abs_node_t, mapping_strategy_action>>;

inline std::vector<cone_t> get_cones(abs_node_t node,
  ...
  xag.foreach_fanin(node, [&](auto f) {
    ...
    if (xag.is_nary_xor(fi) &&
        std::find(drivers.begin(), drivers.end(), fi) == drivers.end()) {
  ...
  std::set_difference(left.begin(), left.end(), right.begin(), right.end(),
                      std::inserter(cones[0].target, cones[0].target.begin()));

  /* set difference */
  std::set_difference(right.begin(), right.end(), left.begin(), left.end(),
                      std::inserter(cones[1].target, cones[1].target.begin()));

  return cones;
}

static inline abs_steps_xag_t gen_steps(abs_node_t node, bool compute,
                                        abstract_xag_network const& xag,
                                        std::vector<uint32_t> const& drivers) {
  ...
  if (xag.is_and(node)) {
    ...
    for (auto cone : cones) {
      if (xag.is_and(cone.root) || xag.is_pi(cone.root) ||
          cone.leaves.size() == 1)
        continue;
      if (cone.target.empty()) {
        parity_steps.push_back(
            {cone.root, compute_action{cone.leaves, std::nullopt}});
      } else {
        parity_steps.push_back(
            {cone.root,
             compute_inplace_action{static_cast<uint32_t>(cone.target[0]),
                                    cone.leaves}});
      }
    }

    comp_steps.insert(comp_steps.begin(), parity_steps.begin(),
                      parity_steps.end());
  }
  ...
  if (xag.is_and(node))
    comp_steps.insert(comp_steps.end(), parity_steps.begin(),
                      parity_steps.end());
...
static inline std::vector<std::vector<abs_node_t>> get_levels_asap(
    abstract_xag_network const& xag_t, std::vector<abs_node_t> const& drivers) {
  mockturtle::depth_view<abstract_xag_network,
                         caterpillar::xag_depth_cost<abstract_xag_network>>
      xag{xag_t};
  ...
  xag.foreach_gate([&](auto const& n) {
    if (xag.is_and(n) ||
        std::find(drivers.begin(), drivers.end(), n) != drivers.end()) {
      auto l = xag.level(n);
...
static inline std::vector<std::vector<abs_node_t>> get_levels_alap(
    abstract_xag_network const& xag_t, std::vector<abs_node_t> const& drivers) {
  mockturtle::depth_view<abstract_xag_network,
                         xag_depth_cost<abstract_xag_network>>
      xag{xag_t};
  ...
  /* compute parents */
  mockturtle::node_map<std::vector<uint32_t>, abstract_xag_network> parents(
      xag);
  xag.foreach_node([&](auto const& n) {
    ...
    xag.foreach_fanin(n, [&](auto const& f) {
      ...
      if (xag.is_and(child) ||
          (std::find(drivers.begin(), drivers.end(), child) != drivers.end())) {
  ...
  for (auto n = xag.size() - 1u; n > 0u; --n) {
    ...
    if (parents[n].empty()) {
      ...
      const auto l =
          level[*std::min_element(
              parents[n].begin(), parents[n].end(),
              [&](auto n1, auto n2) { return level[n1] < level[n2]; })] -
          1u;
      nodes_per_level[l].push_back(n);
      level[n] = l;
    }
...
/*!
  \verbatim embed:rst
    This strategy is dedicated to XAG graphs and fault tolerant quantum
  computing. It exploits two main facts:

    1.  XORs are relatively cheap to be implemented in fault tolerant quantum
  computing
    2.  Toffoli gates visited to implement AND nodes can be uncomputed using 0 T
  gates

    Details can be found in :cite:`MSC19`.
  \endverbatim
*/
class abstract_xag_mapping_strategy
    : public mapping_strategy<abstract_xag_network> {
 public:
  bool compute_steps(abstract_xag_network const& ntk) override {
    ...
    xag.foreach_gate([&](auto node) {
      if (xag.is_and(node) ||
          std::find(drivers.begin(), drivers.end(), node) != drivers.end()) {
...
class abstract_xag_fast_lowt_mapping_strategy
    : public mapping_strategy<abstract_xag_network> {
 public:
  bool compute_steps(abstract_xag_network const& ntk) override {
    mockturtle::topo_view xag{ntk};
    ...
    for (auto lvl : levels) {
      ...
      for (auto node : node_and_action) {
        if (std::find(drivers.begin(), drivers.end(), node.first) ==
            drivers.end())
          to_be_uncomputed.push_back(node);
      }
      it = steps().insert(it,
                          {lvl[0], uncompute_level_action{to_be_uncomputed}});
    }
...
/*!
  \verbatim embed:rst
    This strategy is dedicated to XAG graphs and fault tolerant quantum
  computing. It finds a strategy that aim to minimize the number of T-stages or
  T-depth. Every level uses some extra-qubits to copy AND inputs. \endverbatim
*/
class abstract_xag_low_depth_mapping_strategy
    : public mapping_strategy<abstract_xag_network> {
  void eval_copies(std::vector<cone_t>& cones,
 ...
 public:
  ...
  bool compute_steps(abstract_xag_network const& ntk) override {
    ...
    auto drivers = detail::get_outputs(xag);
    auto levels =
        _alap ? get_levels_alap(xag, drivers) : get_levels_asap(xag, drivers);
      ...
      if (lvl.size() > 0) {
        {
          /* store two action sets for each node in the level */
          std::vector<std::pair<uint32_t, std::vector<cone_t>>> node_and_action;
          std::vector<std::pair<uint32_t, std::vector<cone_t>>>
              to_be_uncomputed;
          ...
          it = steps().insert(it,
                              {lvl[0], compute_level_action{node_and_action}});
          it = it + 1;
          ...
          it = steps().insert(
              it, {lvl[0], uncompute_level_action{to_be_uncomputed}});
        }
      }
    return true;
  }
