...
#pragma region Constructors
  mcmt_gate(gate_base const& op, qubit_id target)
      : gate_base(op), controls_(0), polarity_(0), targets_(0) {
...
#pragma region Properties
  ...
  auto qubit_slot(qubit_id qid) const { return qid.index(); }
#pragma endregion

#pragma region Const iterators
  template <typename Fn>
  void foreach_control(Fn&& fn) const {
    for (auto i = controls_, id = 0u, p = polarity_; i;
         i >>= 1, ++id, p >>= 1) {
      if (i & 1) {
        fn(qubit_id(id, (p & 1)));
      }
...
#pragma endregion
 ...
 private:
  /*! \brief bitmap which indicates which qubits in the network are the
   * controls. */
  uint32_t controls_;
